...
import csv
import logging
logging.basicConfig(filename='bot2_230714.log', encoding='utf-8', level=logging.INFO)
...
def writewrite(lists):
    try:
        with open('P-3_Q1x.csv', 'a+', newline="") as output_file:
...
if __name__ == '__main__':
    cloudpss.setToken('eyJhbGciOiJFUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6NjExLCJ1c2VybmFtZSI6InNvMm9tb24iLCJzY29wZXMiOltdLCJ0eXBlIjoiYXBwbHkiLCJleHAiOjE3MTk5OTMzMzcsImlhdCI6MTY4ODg4OTMzN30.5VOzk2HuY4FDpDzd6B3GJJ3ZZFRfUaVAjfvvnoPFm5H6vhdgW9ahN5YlUvhX0z3BC1hrbFNOoFujIw56n1tx0Q')
    ### 获取指定 rid 的项目
    os.environ['CLOUDPSS_API_URL'] = 'https://internal.cloudpss.net/'
    model = cloudpss.Model.fetch('model/so2omon/verif_VdcIq')
    config = model.configs[0]  # 不填默认用model的第一个config
    job = model.jobs[0]  # 不填默认用model的第一个job

    # ##元件匹配
    ##f
    # f = [3,5,7,9,11,13,17,29,37,49,59,75,81,93,103,117,143,173,193,213]
    ...
    Q = [20*0.4,20*0.5,20*0.6,20*0.7,20*0.8,20*0.9,20*1]
    Q_current_source = model.getComponentByKey('component_new_step_gen_9')
    P = [0.03 * -0.4]
    P_current_source = model.getComponentByKey('component_new_dc_current_source_1')
    for Idc in P:
